...
class User(BaseModel, db.Model):
    ...
    def set_password_hash(self, password):
        ...
        # 2.将加密后的密码赋值给password_hash属性
        self.password_hash = password_hash

    # password属性的get方法--> print(user.password) 触发get方法
    @property
    def password(self):
        AttributeError("密码不允许读取")

    # password属性的set方法--> user.password = "12356" 触发set方法
    @password.setter
    ...
    def check_passowrd(self, password):
        # 参数1：加密后的密码 参数2：未加密的密码  返回值：BOOL
        # 将未加密的密码再次按照相同的加密算法加密后跟加密的密码进行对比
        return check_password_hash(self.password_hash, password)

    # 将用户对象转换成字典  object -- {"id": self.id, "nick_name": self.nick_name,}
    def to_dict(self):
        resp_dict = {
            "id": self.id,
